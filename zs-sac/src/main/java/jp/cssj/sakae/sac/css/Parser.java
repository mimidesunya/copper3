...
public interface Parser {
	 ...
	 *
	 * <p>
	 * CSS parsers are not required to provide localisation for errors and warnings;
	 * if they cannot support the requested locale, however, they must throw a CSS
	 ...
	 *                Throws an exception (using the previous or default locale) if
	 ...
	 * which will throw a CSSException (this is the default behaviour implemented by
	 * HandlerBase).
	 * </p>
	 *
	 * <p>
	 ...
	 * parsing an CSS document from any valid input source (a character stream, a
	 * byte stream, or a URI).
	 * </p>
	 *
	 * <p>
	 * Applications may not invoke this method while a parse is in progress (they
	 * should create a new Parser instead for each additional CSS document). Once a
	...
	/**
	 * Returns a string about which CSS language is supported by this parser. For
	 * CSS Level 1, it returns "http://www.w3.org/TR/REC-CSS1", for CSS Level 2, it
	 * returns "http://www.w3.org/TR/REC-CSS2". Note that a "CSSx" parser can return
	 * lexical unit other than those allowed by CSS Level x but this usage is not
	 * recommended.
	 */
