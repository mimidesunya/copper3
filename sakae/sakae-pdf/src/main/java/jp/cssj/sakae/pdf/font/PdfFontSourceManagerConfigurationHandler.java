...
class PdfFontSourceManagerConfigurationHandler extends DefaultHandler {
	...
	private static void readSystemFont(FontFace face, List<FontSource> list, String types, java.awt.Font font,
			Map<String, CMap> nameToCMap) throws IOException {
		if (types.indexOf("cid-keyed") != -1) {
			list.add(new PdfFontSourceWrapper(FontLoader.readSystemFont(face, FontLoader.TYPE_CID_KEYED, font, nameToCMap)));
		}
		if (types.indexOf("cid-identity") != -1) {
			list.add(new PdfFontSourceWrapper(FontLoader.readSystemFont(face, FontLoader.TYPE_CID_IDENTITY, font, nameToCMap)));
		}
		if (types.indexOf("embedded") != -1) {
			list.add(new PdfFontSourceWrapper(FontLoader.readSystemFont(face, FontLoader.TYPE_EMBEDDED, font, nameToCMap)));
		}
	}
				...
				try {
					if (file != null) {
						...
						try (InputStream in = new FileInputStream(theFile)) {
							...
							readSystemFont(face, list, types, font, this.nameToCMap);
						}
					} else if (dir != null) {
					...
					final String font = src == null ? file : src;
					LOG.log(Level.WARNING, "'" + font + "'のフォント情報の取得に失敗しました。", e);
					this.fontSources = null;
				}
